...
void pdk_save_init(PlaydateAPI *playdate) { pd = playdate; }

int pdk_save_write(const char *filename, const void *data, int size) {
  SDFile *f = pd->file->open(filename, kFileWrite);
  if (f == NULL) {
    pd->system->logToConsole("pdk_save: failed to open '%s' for writing: %s",
                             filename, pd->file->geterr());
    return 0;
  }
  ...
  if (written != size) {
    pd->system->logToConsole("pdk_save: write incomplete (%d of %d bytes)",
                             written, size);
    return 0;
  }
